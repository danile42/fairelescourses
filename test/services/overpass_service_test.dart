...
Map<String, dynamic> _overpassResponse(List<Map<String, dynamic>> elements) =>
    {'version': 0.6, 'elements': elements};

Map<String, dynamic> _node(int id, double lat, double lon, Map<String, dynamic> tags) => {
      'type': 'node',
      'id': id,
      'lat': lat,
      'lon': lon,
...
Map<String, dynamic> _way(int id, double lat, double lon, Map<String, dynamic> tags) => {
      'type': 'way',
      'id': id,
...
void main() {
  group('OverpassService.searchNearby', () {
    test('parses node with lat/lon', () async {
      ...
      final results = await OverpassService.searchNearby(48.0, 11.0, 2000,
          httpClient: client);
    ...
    test('HTTP error throws exception', () async {
      ...
      expect(
        () => OverpassService.searchNearby(48.0, 11.0, 2000,
            httpClient: client),
        throwsException,
      );
    });
    ...
    test('way without center coordinates is skipped', () async {
      final client = _client(_overpassResponse([
        {'type': 'way', 'id': 10, 'tags': {'name': 'Incomplete'}},
      ]));
      final results = await OverpassService.searchNearby(48.0, 11.0, 2000,
          httpClient: client);
      expect(results, isEmpty);
  ...
  group('osmCategoryLabel', () {
    ...
    test('catSupermarket matches list entry', () {
      final cat = osmShopCategories.firstWhere((c) => c.labelKey == 'catSupermarket');
      expect(cat.osmKey, 'shop');
      expect(cat.osmValue, 'supermarket');
    });

    test('catPharmacy uses amenity key', () {
      final cat = osmShopCategories.firstWhere((c) => c.labelKey == 'catPharmacy');
